...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
	...
	if (attackerHoldEffect == HOLD_EFFECT_LIGHT_BALL && (attacker->species == SPECIES_VOLBEAT || attacker->species == SPECIES_ILLUMISE || attacker->species == SPECIES_PLUSLE || attacker->species == SPECIES_MINUN)){
		attack = (150 * attack) / 100;
        spAttack = (150 * spAttack) / 100;
	}
    if (defenderHoldEffect == HOLD_EFFECT_METAL_POWDER && (defender->species == SPECIES_DITTO || defender->species == SPECIES_CASTFORM || defender->species == SPECIES_LUVDISC))
        defense *= 2;
